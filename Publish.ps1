...
# Output goes through Write-Information so a host can capture, redirect or suppress it.
...
if ($runExitCode -ne 0) {
	Write-Information ""
	Write-Information "$Red`The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId$Reset"
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			Write-Information "$Red`Reason: $annotation$Reset"
		}
	}

	Write-Information ""
	Write-Information "$Yellow`Tag $version is pushed but no package was published. Once the cause is fixed:$Reset"
	Write-Information "$Cyan  gh run rerun $runId --repo $repoFullName --failed$Reset"
	exit 1
}

Write-Information "$Green`Package $version published.$Reset"